...
void pos2mu(string nome_pos="ciao",int n_tgt=0,double l_tgt=0,string tgt="be"){
  ...
  if(tgt=="c"){
   ...
   while (1) {
     ...
     //if(!in.good()) break;
     if(partID==2750&&turn==n_tgt-1) break; //BRUTTOOOOOOOO
   }
   in.close();
   ...
   outpos.open(nome_pos,ios::app);
   outmu.open(nome_mu,ios::app);
   E_mu=E/2;
   for(int i=0;i<nentries;i++){
	 ...
	 if(sqrt(S)/2>=m_mu){;
	   ...
	   outmu<<partID_mu<<" "<<turn_mu<<" "<<x_mu<<" "<<px_mu<<" "<<y_mu<<" "<<py_mu<<" "<<ct_mu<<" "<<dE_E_mu<<" "<<s_mu<<" "<<E_mu<<" "<<w<<" "<<z<<" "<<turn0<<endl;
	   
	   //turn=turn+1;
	   //outpos<<partID<<" "<<turn<<" "<<x<<" "<<px<<" "<<y<<" "<<py<<" "<<ct<<" "<<dE_E<<" "<<s<<" "<<E<<endl;
	   //break;
	 }//end if(sqrt(S)/2>m_mu)
	 }//end if(i==zprod)
	 
	 if(/*sqrt(S)/2<m_mu&&*/i==n_step){
	   //cout<<partID<<" "<<turn+1<<endl;
	   turn=turn+1;
	   outpos<<partID<<" "<<turn<<" "<<x<<" "<<px<<" "<<y<<" "<<py<<" "<<ct<<" "<<dE_E<<" "<<s<<" "<<E<<endl;
